...
class Gh615(SerialInterface):
    ...
    def __parseUnitInformation(self, hex):
        if len(hex) == 180:
            unit = {
                'device_name'      : Utilities.hex2chr(hex[4:20]),
                'version'          : Utilities.hex2dec(hex[50:52]),
                #'dont know'       : self.__hex2dec(response[52:56]),
                'firmware'         : Utilities.hex2chr(hex[56:88]),
                'name'             : Utilities.hex2chr(hex[90:110]),
                'sex'              : 'male' if (Utilities.hex2chr(hex[112:114]) == '\x01') else 'female',
                'age'              : Utilities.hex2dec(hex[114:116]),
                'weight_pounds'    : Utilities.hex2dec(hex[116:120]),
                'weight_kilos'     : Utilities.hex2dec(hex[120:124]),
                'height_inches'      : Utilities.hex2dec(hex[124:128]),
                'height_centimeters' : Utilities.hex2dec(hex[128:132]),
                'waypoint_count'   : Utilities.hex2dec(hex[132:134]),
                'trackpoint_count' : Utilities.hex2dec(hex[133:138]),
                'birth_year'       : Utilities.hex2dec(hex[138:142]),
                'birth_month'      : Utilities.hex2dec(hex[142:144])+1,
                'birth_day'        : Utilities.hex2dec(hex[144:146])
            }
            return unit
        else:
            raise Gh615ParseException
