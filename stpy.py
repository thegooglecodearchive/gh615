...
class TemplateHTML(Template):
    ...
    def __init__(self, source):        
        fileHandle = open('gui/'+source)
        template = fileHandle.read()
        fileHandle.close() 
        
        fileHandle = open('gui/global/header.html')
        header = fileHandle.read()
        fileHandle.close() 
        
        fileHandle = open('gui/global/footer.html')
        footer = fileHandle.read()
        fileHandle.close() 
        
        template =  header + template + footer
        self.parentInit(template)
        
    def render(self, cherrypy, msg = '', *args, **kwargs):            
        lines = []
        d = dict(*args, **kwargs)
        
        d['config'] = cherrypy.config
        
        if cherrypy.session.get('flash') is not None:
            d['flash'] = cherrypy.session.get('flash')
            cherrypy.session.pop('flash')
        elif msg:
    ...
    def redirect(self, cherrypy, url, msg='', *args, **kwargs):            
        lines = []
        d = dict(*args, **kwargs)
